...
[AddComponentMenu("KLabsAudioTools/MusicScript")]
public class MusicScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        segmentSourceA.outputAudioMixerGroup = m_outputMixerGroup;
    ...
    public void musicTrigger(bool trigger, GameObject objTrigger, AudioClip clip, int barLength)
    {
        ...
        if (trigger) //if enters next segment gameObject
        {
            playing = true;

            if(loopNumber < 1) loopNumber = 1;

            nextEventTime += (60.0f / m_userBpm * barLength) * (loopNumber); //program the next transition
        ...
        else //if enters previous segment gameObject
        {
            nbTrigger -= 1;
